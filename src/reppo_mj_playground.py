...
def make_train_fn(cfg: DictConfig) -> Callable[[jax.random.PRNGKey], tuple[REPPOTrainState, dict[str, jax.Array]]]:
    ...
    def compute_extras(key: jax.random.PRNGKey, train_state: REPPOTrainState, batch: Transition):
        ...
        # We need log pi(a_t+1 | s_t+1). Due to auto-resets, we do not have actions for truncated steps,
        # so we need to sample next actions for those steps and use the log prob of those actions instead,
        # while for non-truncated steps we can use the log prob of the actual next action taken in the environment.
        ...
        # compute soft reward for actor update
        soft_reward = batch.reward - cfg.gamma * true_next_log_prob * actor_model.temperature()

        # We also need the value of the next state for the critic update, and for truncated steps we should use the value of the actual next state,
        # instead of the value of the auto-reset state, so we also compute that here.
        key, next_act_key = jax.random.split(key)
        # We approximate V(s_t+1) with a monte-carlo estimate using the critic, by sampling an action for the next state and evaluating it with the critic.
    ...
    def train_eval_loop_body(train_state: REPPOTrainState, key: jax.random.PRNGKey) -> tuple[REPPOTrainState, dict]:
        ...
        jax.debug.callback(util.log_callback, train_state, metrics)
        return train_state, metrics
...
@hydra.main(version_base=None, config_path=".", config_name="reppo_mj_playground")
