...
from src.algorithms.ppo.ppo import PPOConfig, PPOTrainState, make_ppo_init_fn, ppo_policy_fn

logging.basicConfig(level=logging.INFO)


class DPOConfig(PPOConfig):
    alpha: float = 2.0
    beta: float = 0.6


def make_dpo_learner_fn(cfg: DPOConfig):
...
def main(cfg: DictConfig):
    ...
    dpo_cfg = DPOConfig(**cfg.hyperparameters)
    ...
    train_fn = make_train_fn(
        ...
        init_fn=make_ppo_init_fn(dpo_cfg, env, env_params),
        learner_fn=make_dpo_learner_fn(dpo_cfg),
