...
class Normalizer:
    ...
    def update(self, state: NormalizationState, tree: struct.PyTreeNode) -> NormalizationState:
        ...
        mean, var, count = jax.tree.transpose(jax.tree.structure(tree), jax.tree.structure(("*", "*", "*")), stats)
    ...
    def normalize(self, state: NormalizationState, tree: struct.PyTreeNode) -> struct.PyTreeNode:
        return jax.tree.map(lambda x, m, v: (x - m) / jnp.sqrt(v + 1e-8), tree, state.mean, state.var)
