...
uv run src/train.py --config-name=reppo_maniskill algorithm=reppo env=maniskill env.name=${env[$((SLURM_ARRAY_TASK_ID%8))]} tags=[maniskill6,langevin_sqrt] logging=wandb_online seed=$RANDOM +experiments=maniskill algorithm.policy_method=langevin algorithm.mask_truncated=False
