...
def gaussian_log_prior(
    x, mu=jnp.zeros((2,)), cov=(jnp.array([[1.0, 0.0], [0.0, 0.1]]).dot(rotation_45.T))
):
    d = x.shape[0]
    x_centered = x - mu
...
def hump_energy(
    x,
    centroids=jnp.array([[1.0, 1.0], [-1.4, -1.4]]),
    scales=jnp.array([0.6, 0.6]),
    weights=jnp.array([0.6, 0.6]),
):
    # Mixture of Gaussians energy landscape (unnormalized)
    # E(x) = - log( sum_k w_k * N(x; c_k, s_k^2 I) )
    exps = jnp.array(
        [
            w * jnp.exp(-0.5 * jnp.sum((x - c) ** 2) / (s**2))
            for (c, s, w) in zip(centroids, scales, weights)
        ]
    )
    return -jnp.log(jnp.sum(exps) + 1e-8)


def precond_mala_posdep(
    key, x0, n_steps, eps, E, log_prior, G_fn, tau=1.0, lambda_scalar=1.0
):
    d = x0.shape[0]
...
if __name__ == "__main__":
    ...
    prior_dist = distrax.MultivariateNormalFullCovariance(
        loc=jnp.zeros((2,)), covariance_matrix=rotated_covariance
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/voelcke1/reppo/langevin_visualization.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
